                    ...
                    <table class="table table-striped table-bordered table-hover" id="sample_1">
                        ...
                        <tbody>
                            @foreach ($result as $rs)
                                <tr>
                                    <td>{{ ($rs->nome) }}</td>
                                    <td>{{ ($rs->tipo_produto) }}</td>
                                    <td>{{ ($rs->fornecedor) }}</td>
                                    <td>{{ ($rs->estoque) }}</td>
                                    <td>{{ Util::number($rs->valor, 2) }}</td>
                                    <td>{{ number_format($rs->de_pontos, 0 , '','.') }}</td>
                                    <td>{{ number_format($rs->por_pontos, 0 , '','.') }}</td>
                                    <td>{{ Util::formatStatus($rs->status) }}</td>
                                    <td>{{ Util::toTimestamps($rs->updated_at) }}</td>
                                    <td>
                                        {{ link_to(Route('get-cadastra-estoque', $rs->id), 'Gerenciar', array('class' => 'btn btn-success btn-sm', 'title' => 'Lançamento')) }}</td>
                                    </td>
                                    <td>
